...
@registry.register_model("img_embed")
class ImageWrapEmbeddings(Blip2Base):
    """
    ...
    def __init__(
        ...
        #num_query_token=32,
        llama_model="",
        prompt_path="/scratch/slurm-user3/changsun/minigpt4/prompts/alignment.txt",
        prompt_template="",
        max_txt_len=32,
        end_sym='\n',
        ...
        #dir_name = '/scratch/slurm-user3/changsun/data/ct-image-prompt-wrap/'
    ): 
        super().__init__()
    ...
    def encode_img(self, image):
        ...
        with self.maybe_autocast():
            ...
            image_cls_tokens = image_embeds #image_cls_tokens = image_embeds[:,-1,:]
            inputs_llama = self.llama_proj_chz(image_cls_tokens)
            print("inputs_llama.shape", inputs_llama.shape)
            atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(image.device)
            print('1-1', atts_llama.device)
            inputs_llama = inputs_llama.unsqueeze(dim=1)
            atts_llama = atts_llama.unsqueeze(dim=1)
            print('1-2', atts_llama.device)
    ...
    def forward(self, samples):
        ...
        #file_name = os.path.join(self.dir_name, f"embeddings_{timestamp}.pt")
        
        # Save inputs_embeds and attention_mask to the file
        #torch.save({"inputs_embeds": inputs_embeds, "attention_mask": attention_mask}, file_name)
        save_dict = {"inputs_embeds": inputs_embeds, "attention_mask": attention_mask}
        # Print the saved file name for reference
        print('inputs_embeds.shape', inputs_embeds.shape)
        print('attention_mask.shape', attention_mask.shape)
        
        #print(f"Saved embeddings and attention mask to {file_name}")
